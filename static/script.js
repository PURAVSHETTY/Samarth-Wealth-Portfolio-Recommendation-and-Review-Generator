document.addEventListener("DOMContentLoaded", () => {
    ...
    async function triggerAiProposalGeneration() {
        ...
        const timeoutId = setTimeout(() => {
            console.warn("[DEBUG WARNING] Proposal generation timed out after 180 seconds. Aborting request.");
            controller.abort();
        ...
        try {
            ...
            if (contentType && contentType.includes("application/json")) {
                ...
                console.error("[DEBUG ERROR] Non-JSON response received from server. Complete response body:");
                console.error(text);
                throw new Error("Server returned non-JSON response. Check browser console for traceback.");
            }
